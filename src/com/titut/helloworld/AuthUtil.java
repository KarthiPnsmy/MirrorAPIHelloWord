...
public class AuthUtil {
	...
	public static AuthorizationCodeFlow newAuthorizationCodeFlow()
			throws IOException {
		/*
		 * URL resource = AuthUtil.class.getResource("/oauth.properties"); File
		 * propertiesFile = new File("./src/main/resources/oauth.properties");
		 * try { propertiesFile = new File(resource.toURI());
		 * //LOG.info("Able to find oauth properties from file."); } catch
		 * (URISyntaxException e) { LOG.info(e.toString());
		 * LOG.info("Using default source path."); } FileInputStream
		 * authPropertiesStream = new FileInputStream(propertiesFile);
		 * Properties authProperties = new Properties();
		 * authProperties.load(authPropertiesStream);
		 * 
		 * String clientId = authProperties.getProperty("client_id"); String
